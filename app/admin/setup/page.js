...
const GITHUB_REPO = process.env.NEXT_PUBLIC_GITHUB_REPO || 'hjcodingwoo/manual-of-me'
...
export default function SetupPage() {
  ...
  const pushCompaniesToGitHub = async (updatedCompanies) => {
    setDeploying(true)
    try {
      // Build companies.json object keyed by slug
      const companiesObj = {}
      ...
      const content = JSON.stringify(companiesObj, null, 2)
      const encoded = btoa(unescape(encodeURIComponent(content)))

      // Get current SHA of companies.json
      const token = process.env.NEXT_PUBLIC_GITHUB_TOKEN
      const getRes = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/companies.json`, {
      ...
      // Push updated file
      await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/companies.json`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json', 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: `Update companies.json — add/edit ${updatedCompanies.map(c=>c.slug).join(', ')}`, content: encoded, sha })
      })
      setMsg(prev => prev + ' · GitHub updated · Vercel redeploying (~60s)')
    } catch(e) {
      setMsg(prev => prev + ' · GitHub push failed — update companies.json manually')
    }
    setDeploying(false)
  }
