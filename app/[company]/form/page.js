...
export default function FormPage() {
  ...
  useEffect(() => {
    ...
    if (!name) { router.push(`/${slug}`); return }
    setUserName(name)
    ...
    <div style={{ minHeight:'100vh', padding:'clamp(1rem,3vw,1.5rem)', background:tertiary, fontFamily:'Georgia,serif', color:'#1a1a18' }}>
      <div style={{ maxWidth:'800px', margin:'0 auto' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1rem' }}>
          <button onClick={() => router.push(`/${slug}`)} style={{ background:'transparent', border:'1px solid #ddd', color:'#666', padding:'0.4rem 0.875rem', borderRadius:'8px', fontSize:'13px', cursor:'pointer' }}>← Home</button>
          {savedMsg && <span style={{ fontSize:'13px', color:primary, fontWeight:'bold' }}>{savedMsg}</span>}
        </div>

        {currentQ === 8 && (
          <div style={{ background:primary, color:'white', borderRadius:'10px', padding:'0.75rem 1rem', marginBottom:'1rem', fontSize:'13px', textAlign:'center' }}>
        ...
        {isFavPage && (
          <>
            <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'1.5rem' }}>
              <span style={{ fontSize:'11px', fontWeight:'bold', background:secondary, color:'#1a1a18', padding:'2px 8px', borderRadius:'10px', textTransform:'uppercase' }}>Fun</span>
          ...
          <>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'6px', marginBottom:'12px' }}>
              {workQ.chips.map((chip, i) => {
                ...
                return <button key={i} onClick={() => toggleChip(chip)} style={{ padding:'6px 12px', background: active ? '#1a1a18' : 'white', color: active ? '#FFBF00' : '#1a1a18', border: active ? `1px solid ${primary}` : '1px solid #e0d8c8', borderRadius:'20px', fontSize:'13px', cursor:'pointer', transition:'all 0.15s' }}>{chip}</button>
              })}
            </div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'1.25rem' }}>
          ...
          <button onClick={goNext} style={{ padding:'0.75rem 1.5rem', background:primary, color:'white', border:'none', borderRadius:'8px', fontSize:'14px', fontWeight:'bold', cursor:'pointer' }}>
